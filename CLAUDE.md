...
`maintain-feed-size.yml` が定期実行で `feed.json` を保持上限（既定 100 件）に収まるよう `publishedAt` の古い順に自動削除し、孤立画像も消して PR 経由（自動 squash マージ）で main へ反映する。受信側の DB 使用量がフィードのキュレーションに依存するための責務（詳細は README「フィードサイズの自動メンテナンス」）。
