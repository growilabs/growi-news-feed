...
- 削除後に `npm run validate` を通し、**PR を作成する（マージは運用担当者が手動で行う）**。人がマージ＝main への push なので `pages.yml` が自然に走り Pages に反映される。差分が PR として残るため確認・revert しやすい。運用実績ができたら自動マージ化を検討する。
