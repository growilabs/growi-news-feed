...
- ファイル名は `^[A-Za-z0-9][A-Za-z0-9._-]*\.(png|jpe?g|webp|gif)$`。スペース・日本語は不可で、URL エスケープしても通らない（解決後のパスに `%` が含まれると弾かれる）。アニメーションは **GIF のみ**（mp4 / `<video>` は非対応）。
...
`maintain-feed-size.yml` が定期実行で `feed.json` を保持上限（既定 100 件）に収まるよう `publishedAt` の古い順に自動削除し、孤立画像も消して **PR を作成する（マージは運用担当者が手動）**。自動マージは運用実績ができてから検討。受信側の DB 使用量がフィードのキュレーションに依存するための責務（詳細は README「フィードサイズの自動メンテナンス」）。
